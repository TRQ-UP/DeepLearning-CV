...
class yoloLoss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        """
        pred_tensor: (tensor) size(batchsize,S,S,Bx5+20=12) [x,y,w,h,c]
        target_tensor: (tensor) size(batchsize,S,S,12)
        """
        N = pred_tensor.size()[0]
        # 具有目标标签的索引(bs, 7, 7, 12)中7*7方格中的哪个方格包含目标
        ...
        #  coo_pred：tensor[, 12](所有batch数据都压缩在一起)
        coo_pred = pred_tensor[coo_mask].view(-1, 12)
        box_pred = coo_pred[:, :10].contiguous().view(-1, 5)  # box[x1,y1,w1,h1,c1], [x2,y2,w2,h2,c2]
        class_pred = coo_pred[:, 10:]
        
        coo_target = target_tensor[coo_mask].view(-1, 12)
        box_target = coo_target[:, :10].contiguous().view(-1, 5)
        class_target = coo_target[:, 10:]

        # compute not contain obj loss
        noo_pred = pred_tensor[noo_mask].view(-1, 12)
        noo_target = target_tensor[noo_mask].view(-1, 12)
        ...
        # coo_response_mask = torch.ByteTensor(box_target.size()).bool()
        coo_response_mask.zero_()
        coo_not_response_mask = torch.cuda.ByteTensor(box_target.size()).bool()
        # coo_not_response_mask = torch.ByteTensor(box_target.size()).bool()
        coo_not_response_mask.zero_()
        box_target_iou = torch.zeros(box_target.size()).cuda()
        # box_target_iou = torch.zeros(box_target.size()).to("mps")
        for i in range(0, box_target.size()[0], 2):  # choose the best iou box
            ...
            # max_index = max_index.data.to("mps")
