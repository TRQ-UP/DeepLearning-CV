...
class yoloDataset(data.Dataset):
    ...
    def __init__(self, root, list_file, train, transform):
        ...
        with open(list_file) as f:
            lines = f.readlines()
        for line in lines:
            splited = line.strip().split()  # ['abs_path/005246.jpg', ''80,217,320,273,10'', ''197,193,257,326,8'] ["图片绝对路径","坐标 + 类型（labels）", ...]
            self.fnames.append(splited[0])
            num_boxes = (len(splited) - 1)
            box = []
            label = []
            for i in range(num_boxes):
                x = float(splited[i+1].split(',')[0])
                y = float(splited[i+1].split(',')[1])
                x2 = float(splited[i+1].split(',')[2])
                y2 = float(splited[i+1].split(',')[3])
                c = float(splited[i+1].split(',')[4])
    ...
    def __getitem__(self, idx):
        ...
        # 编辑yolov1的标准输出，用于yolo_loss的计算（论文grid_num=7，30=5(c+h+w+h偏移+w偏移)+20(检测的类别))
        # 重点理解
